...
export class PacienteComponent implements OnInit {
  ...
  signosByPaciente: number = 0;

  constructor(
    private pacienteService: PacienteService,
    private signos: SignosService,
  ...
  ngOnInit(): void {
    this.pacienteService.listarPageable(0, this.pageSize).subscribe(data => {
    ...
    this.pacienteService.getPacienteCambio().subscribe(x => {
      this.pacienteService.listarPageable(0, this.pageSize).subscribe(data => {
        this.dataSource = new MatTableDataSource(data.content);
      this.dataSource.sort = this.sort;
      // this.dataSource.paginator = this.paginator;
  ...
  navegaSignos(idPaciente: number){
    this.dataService.idPaciente = idPaciente;
