...
export class WizardComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.segundoFormGroup = this.formBuilder.group({
      // hidden: [Object, Validators.required]
    });

    this.terceroFromGroup = this.formBuilder.group({
  ...
  nextManualStepperMedico() {
    if(this.stepper.linear){
      if(this.medicoSeleccionado === null){
        this.snackBar.open('DEBES SELECCIONAR UN MEDICO', 'INFO', { duration: 2000 });
      } else {
        this.stepper.next();
      }
    } else {
